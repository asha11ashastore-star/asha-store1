#!/usr/bin/env python3
"""
Test the raw dashboard products endpoint
"""
...
if login_response.status_code == 200:
    ...
    # Test raw dashboard endpoint
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get("http://localhost:8000/api/v1/products/dashboard-raw", headers=headers)
    
    print(f"Raw dashboard endpoint status: {response.status_code}")
